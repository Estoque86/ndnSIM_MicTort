...
stringOutput = 'DATA_SIM=NDNSIM_T=SINGLE_CACHE_A=0.6_L=25_100_DROP_REQ_=1000000.mat';
...
folder='/home/tortelli/ndn-simulator-comparison/ndnSIM/ns-3/logs/logs/';
ext='.out';
...
pHit = struct;
for g=1:length(numAlphas)
    pHit.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 5);
end
...
for g=1:length(numAlphas)
    ...
    pHit.(alphaStrCompl{numAlphas(g)})(:,5) = Che_Approx.(alphaStrCompl{numAlphas(g)}){1}(:,1);
...
for g=1:length(numAlphas)
    ...
    norm.(alphaStrCompl{numAlphas(g)}) = [pHit.(alphaStrCompl{numAlphas(g)})(:,5), pHit.(alphaStrCompl{numAlphas(g)})(:,5), pHit.(alphaStrCompl{numAlphas(g)})(:,5), pHit.(alphaStrCompl{numAlphas(g)})(:,5), pHit.(alphaStrCompl{numAlphas(g)})(:,5)];
    pHitNormalized.(alphaStrCompl{numAlphas(g)}) = zeros(catalog,5);
end
...
for g=1:length(numAlphas)
    ...
    fForPhit.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    fForPhitNormalized.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end

for g=1:length(numAlphas)
    for j = 1:length(numLambdas)
        for z = 1:catalog
            fForPhit.(alphaStrCompl{numAlphas(g)})(z,j) = (pHit.(alphaStrCompl{numAlphas(g)})(z,5) - pHit.(alphaStrCompl{numAlphas(g)})(z,j)) / pHit.(alphaStrCompl{numAlphas(g)})(z,5);
            fForPhitNormalized.(alphaStrCompl{numAlphas(g)})(z,j) = (pHitNormalized.(alphaStrCompl{numAlphas(g)})(z,5) - pHitNormalized.(alphaStrCompl{numAlphas(g)})(z,j)) / pHitNormalized.(alphaStrCompl{numAlphas(g)})(z,5);
        end
    end
end
...
for g=1:length(numAlphas)
    ...
    fForPhitWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    fForPhitNormalizedWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end
...
for g=1:length(numAlphas)
    ...
    expectFforPhitWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    expectFforPhitNormalizedWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end
...
for g=1:length(numAlphas)
    ...
    expectAbsFforPhitWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    expectAbsFforPhitNormalizedWeighted.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end
...
for g=1:length(numAlphas)
   ...
   % expectFforPhitNormalized.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 3);
    expectFforPhit.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    expectFforPhitNormalized.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end
...
for g=1:length(numAlphas)
    ...
    expectAbsFforPhit.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
    expectAbsFforPhitNormalized.(alphaStrCompl{numAlphas(g)}) = zeros(catalog, 4);
end
...
f50_REQ_1mln_DROP = struct;
for g=1:length(numAlphas)
    for j = 1:length(numLambdas)
        f50_REQ_1mln_DROP.(alphaStrCompl{numAlphas(g)}).(lambdaStrCompl{numLambdas(j)}) = expectAbsFforPhit.(alphaStrCompl{numAlphas(g)})(cont50.(alphaStrCompl{numAlphas(g)}),j);
    end
end

f75_REQ_1mln_DROP = struct;
for g=1:length(numAlphas)
    for j = 1:length(numLambdas)
        f75_REQ_1mln_DROP.(alphaStrCompl{numAlphas(g)}).(lambdaStrCompl{numLambdas(j)}) = expectAbsFforPhit.(alphaStrCompl{numAlphas(g)})(cont75.(alphaStrCompl{numAlphas(g)}),j);
    end
end

f90_REQ_1mln_DROP = struct;
for g=1:length(numAlphas)
    for j = 1:length(numLambdas)
        f90_REQ_1mln_DROP.(alphaStrCompl{numAlphas(g)}).(lambdaStrCompl{numLambdas(j)}) = expectAbsFforPhit.(alphaStrCompl{numAlphas(g)})(cont90.(alphaStrCompl{numAlphas(g)}),j);
    end
end
...
for g=1:length(numAlphas)
    for j = 1:length(numLambdas)
        f95_REQ_1mln_DROP.(alphaStrCompl{numAlphas(g)}).(lambdaStrCompl{numLambdas(j)}) = expectAbsFforPhit.(alphaStrCompl{numAlphas(g)})(cont95.(alphaStrCompl{numAlphas(g)}),j);
    end
end
save(stringOutput);
