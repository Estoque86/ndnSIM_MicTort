...
do
  ...
  # /usr/bin/time -f "\n%E \t elapsed real time \n%U \t total CPU seconds used (user mode) \n%S \t total CPU seconds used by the system on behalf of the process \n%M \t memory (max resident set size) [kBytes] \n%x \t exit status" -o ${infoDir}/Info_SIM\=${sim}_T\=${net}_M\=${M}_C\=${C}_L\=${L}_A\=${A}_R\=${i}.txt $main -u Cmdenv -f $iniFile -r $i > $logDir/SIM\=${sim}_T\=${net}_M\=${M}_C\=${C}_L\=${L}_A\=${A}_R\=${i}-temp.out 2>&1   
   /usr/bin/time -f "\n%E \t elapsed real time \n%U \t total CPU seconds used (user mode) \n%S \t total CPU seconds used by the system on behalf of the process \n%M \t memory (max resident set size) [kBytes] \n%x \t exit status" -o ${infoDir}/Info_SIM\=${sim}_T\=${T}_M\=${M}_C\=${C}_L\=${L}_A\=${A}_R\=${i}.txt $main --run "scratch/ndn-single-cache-cs-tracers_NEW --catalogCardinality\=${M} --cacheToCatalogRatio\=${C} --lambda\=${L} --alpha\=${A} --plateau\=${plateau} --simType\=${T} --simDuration\=${simDuration} --RngSeed\=1 --RngRun\=${j}" > $logDir/stdout/SIM\=${sim}_T\=${T}_M\=${M}_C\=${C}_L\=${L}_A\=${A}_R\=${i}.out 2>&1   
