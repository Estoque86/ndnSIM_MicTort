...
struct Stats
{
  inline void Reset ()
  {
    m_cacheHits   = 0;
    m_cacheMisses = 0;
   	m_intStatsHits.clear();
   	m_intStatsMisses.clear();
  }
  double m_cacheHits;
  double m_cacheMisses;
  std::multimap<uint32_t, uint32_t> m_intStatsHits;
  std::multimap<uint32_t, uint32_t> m_intStatsMisses;
};
/// @endcond
...
public:
  ...
  std::string
  PrintIntNames(std::string fieldType) const;

  std::string
  PrintDistances(std::string fieldType) const;
